...
function calc(el) {
  ...
  if (f[index_f].power != -1) {
	...
	<text style="color:gray"><text style="font-size:20px">
		장판 : <strong>${(((desvioByDegree(f[index_f].desvio, distance) / 0.21656) * dis)*0.8476487854 / aim).toFixed(3)}</strong><br>
<text style="color:green"><text style="font-size:35px"><strong>스마트 : ${(((desvioByDegree(f[index_f].desvio, distance) / 0.21656) * dis)*0.8476487854*0.25 / aim).toFixed(2)} </strong>
	</text><br>
...
<text style="color:gray"><text style="font-size:16px">
		(스마트 2.5칸이 장판 10.0칸 / 화면끝은 좌우로 2.52칸)
		</text><br>`;

  } else {
    result.color = 'DarkRed'
    result.innerHTML = 'N찾o conseguiu calcular a tacada, n찾o chega ou 챕 muito fraco.'
  }
}
